    ...
    parsers: {
      ...
      object: function object(map, stack) {
        ...
        for (var _i = 0; _i < keys.length; _i++) {
          ...
          var val = map[_key2];
          ret.push(
            dump.parse(_key2, 'key') + ': ' + dump.parse(val, undefined, stack),
          );
        }
        dump.down();
        return join('{', ret, '}');
    ...
    [
        ...
        value: function onTestEnd(test) {
          ...
          if (test.status === 'passed') {
            ...
            this.log(
              'ok '
                .concat(this.testCount, ' ')
                .concat($.yellow(test.fullName.join(' > ')), ' # SKIP'),
            );
          } else if (test.status === 'todo') {
            this.log(
              'not ok '
                .concat(this.testCount, ' ')
                .concat($.cyan(test.fullName.join(' > ')), ' # TODO'),
            );
      ...
      },
      {
        ...
        value: function onRunEnd(runEnd) {
          ...
          this.log(
            '# '.concat($.yellow('skip '.concat(runEnd.testCounts.skipped))),
          );
          this.log('# '.concat($.cyan('todo '.concat(runEnd.testCounts.todo))));
          this.log(
            '# '.concat($.red('fail '.concat(runEnd.testCounts.failed))),
          );
        },
      },
      {
      ...
      },
      {
        ...
        value: function logError(error) {
          ...
          if (error && error.stack) {
            var fmtStack = annotateStacktrace(
              error.stack,
              $.grey,
              error.toString(),
            );
      ...
      value: function add(testTasksFunc, prioritize) {
        if (prioritize) {
          ...
          var index = Math.floor(
            this.unitSampler() * (config.queue.length - this.priorityCount + 1),
          );
...
(function () {
  ...
  if (urlParams.seed === 'true' || urlParams.seed === true) {
    ...
    QUnit.config.seed = (Math.random().toString(36) + '0000000000').slice(
      2,
      12,
    );
